...
Static context for this repo. It is loaded on every turn, so it stays short — the
same trade-off `context.py` makes for the agent we are building. Add a rule here when
something goes wrong twice; delete one when it stops being true.
